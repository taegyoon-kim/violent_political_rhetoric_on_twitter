...
toxicity_corpus = pd.read_csv("/Users/taegyoon/Google Drive/jigsaw-unintended-bias-in-toxicity-classification/train.csv") 
toxicity_corpus.head()
toxicity_corpus.shape
toxicity_corpus = toxicity_corpus[toxicity_corpus['comment_text'].apply(lambda x: len(x)<281)]
toxicity_corpus.shape
...
lr = LogisticRegression(C=5, random_state=1, solver='sag', max_iter=1000, n_jobs=-1)
lr.fit(X, y) 
...
# the list of initial violent keywords (n=200)
